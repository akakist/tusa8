...
{MUTEX_INSPECTOR;
	char ss[200];
	string s;
	snprintf(ss, sizeof(ss) - 1, "%2.2f", i);
	s = ss;
	return s;
}
...
{MUTEX_INSPECTOR;
XTRY
	if(strstr(strupper(str).c_str(),strupper(patt).c_str())!=NULL) return true;
	return false;
//	if(strupper(str.substr(0,patt.size()))==strupper(patt)) return true;
XPASS
	return false;
}
...
string strlower(const std::string & s)
{
	std::string a;
XTRY	
	for (unsigned int i = 0; i < s.length(); i++) {
